...
# z_theta(x_t, t) to predict noise z_t
class ZThetaMLP(nn.Module):
    ...
    def __init__(
        ...
        self.time_emb = nn.Embedding(T, hidden_size)
        self.output_layer = nn.Linear(hidden_size, input_size)

    def forward(self, x, t):
        hidden = self.input_layer(x) + self.time_emb(t)
...
class UNet(nn.Module):
    def __init__(self, T, input_ch, ch, ch_mult, attn, num_res_blocks, dropout):
        super().__init__()
...
class DiffusionModel(nn.Module):
    def __init__(
        ...
        if model is None:
            model = UNet(
                T=T, ch=64, ch_mult=[1, 2, 2], attn=[1],
    ...
    def sample(self, xT, verbose=False, return_hiddens=False):
        # used for sampling
        # input x_T ~ N(0,I)    [batch, channel, width, height]
        xt = xT
        ...
        with torch.no_grad():
            for time_step in reversed(range(self.T)):
                ...
                mean, var = self.p_mean_variance(x_t=xt, t=t)
                # no noise when t == 0
                if time_step > 0:
